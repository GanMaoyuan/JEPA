...
针对JEPA，“使训练数据点的能量相对较低”这一训练子目标实际上可以被等价解读为“使JEPA的前向输出 ![](https://latex.codecogs.com/svg.latex?\tilde{s}_y=\text{Pred}(s_x,z)) 在训练过程中逐渐趋同于 ![](https://latex.codecogs.com/svg.latex?s_y) ”，那么该训练子目标对应的损失项实际上即为
